...
*/
uiSleep (random [1,3,6]); //Geld Glitch-Patch 1-6 sek. sollte reichen ^^
private ["_value"];
_value = parseNumber(ctrlText 2702);
private _bankuser = group player getVariable ["gbank_in_use_by",nil]; 

IF (isNil "_bankuser") then {group player setVariable ["gbank_in_use_by",player,true];};
...
CASH = CASH - _value;
_gFund = GANG_FUNDS;
//uiSleep (30 + random(180));		//Geld Glitch-Patch
...
group player setVariable ["gbank_in_use_by",nil,true]; 
